<?php
    ...
    if($_POST['delete']){
      // $query = "delete from lojas where codigo = '{$_POST['delete']}'";
      $query = "update lojas set deletado = '1' where codigo = '{$_POST['delete']}'";
      sisLog($query);
    }

    if($_POST['situacao']){
      $query = "update lojas set situacao = '{$_POST['opc']}' where codigo = '{$_POST['situacao']}'";
      sisLog($query);
      exit();
    }
    ...
    if($_SESSION['usuarioBusca']){
      $busca = str_replace( ['.','-','/',' '], false, $_SESSION['usuarioBusca']);
      $where = " and nome like '%{$_SESSION['usuarioBusca']}%' or REPLACE( REPLACE( REPLACE( cnpj, '/', '' ), '.', '' ), '-', '' ) = '{$busca}' or REPLACE( REPLACE( REPLACE( REPLACE( telefone, ' ', '' ), '(', '' ), ')', '' ), '-', '' ) = '{$busca}' ";
    }
...
<div class="col">
  <div class="m-3">
    ...
    <div class="row">
      <div class="col">
        <div class="card">
          <h5 class="card-header">Lista de Lojas</h5>
          <div class="card-body">
            ...
            <div class="table-responsive d-none d-md-block">
              <table class="table table-striped table-hover">
                ...
                <tbody>
                  <?php
                    $query = "select * from lojas where deletado != '1' {$where} order by nome asc";
                    $result = sisLog($query);
