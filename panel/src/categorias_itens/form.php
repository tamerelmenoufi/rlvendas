<?php
    ...
    if($_POST['acao'] == 'salvar'){
        ...
        if($_POST['codigo']){
            $query = "update categorias_itens set {$attr} where codigo = '{$_POST['codigo']}'";
            sisLog($query);
            $cod = $_POST['codigo'];
        }else{
    ...
    $query = "select * from categorias_itens where codigo = '{$_POST['cod']}'";
    $result = sisLog($query);
    ...
    <form id="form-<?= $md5 ?>">
        <div class="row">
            <div class="col">
                ...
                ?>
                <center><img src="src/categorias_itens/icon/<?=$d->icon?>" style="margin: 20px;" /></center>
                <?php
                }
                ?>
